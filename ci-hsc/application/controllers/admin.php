...
class Admin extends CI_Controller{
    ...
    function add_daily_sales(){
        ...
        $this->session->set_userdata('show','add_sales');
    ...
    function daily_binding_add(){
        ...
        if(!$total_sale){
            $this->session->set_flashdata('alert_type','warning');
            $this->session->set_flashdata('alert_msg',
                'You have to insert your <strong>Total Daily Binding</strong>');
            $this->add_daily_sales();
            ...
            // INSERT the Total Binding in the Database
            $inserted_results = $this->admin_->add_daily_binding($user_id,$branch_id,$date,$total_sale);
            ...
            if($inserted_results){
                ...
                if($check>=1){
                    ...
                    $this->session->set_flashdata('alert_msg',"Sorry ".make_me_bold($full_name).", we have the daily binding for ".make_me_bold($date).", Try editting.");
                    $msg="Failed to add Daily Binding : Tsh $total_sale  for $date by $full_name,since we can only add 1 binding per day.";
                    $this->session->set_flashdata('alert_type','warning');
                }
                $this->usuals->log_write($user_id,$branch_id,$msg);
                //var_dump($this->session->flashdata('alert_msg'));
    ...
    function edit_daily_sales(){
        $this->session->set_userdata('show','edit_sales');
        redirect(base_url('hsc/daily_sales'));
    }
    ...
    function edit_daily_binding(){
        $this->session->set_userdata('show','edit_binding');
        redirect(base_url('hsc/daily_sales'));
    }
